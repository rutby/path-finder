export namespace RTS {
    export namespace PathFinder {
        ...
        export class MiscUtils {
            ...
            public static vec2deg(vec: cc.Vec2): number {
        ...
        export class Dijkstra {
            ...
            //================================================ public
            public constructor(mapSize: cc.Size, gridSize: cc.Size) {
            ...
            public convertViewPosToMapPos(viewPos: IPos): cc.Vec2 {
                return cc.v2(Math.floor(viewPos.x / this._gridSize.width), Math.floor(viewPos.y / this._gridSize.height))
            }
                
            /** 格子坐标转换成UI坐标 */
            public convertMapPosToViewPos(mapPos: IPos): cc.Vec2 {
                return cc.v2(mapPos.x * this._gridSize.width, mapPos.y * this._gridSize.height);
            }
            
            /** 格子坐标转换成索引 */
            public convertMapPosToIndex(mapPos: IPos) {
                return (mapPos.y * this._mapSize.width) + mapPos.x;
            }
            
            /** 索引转换成格子坐标 */
            public convertIndexToMapPos(index: number): IPos {
                var y = Math.floor(index / this._mapSize.width);
                var x = index % this._mapSize.width;
                return {x: x, y: y}
            }
            
            /** 取四/八方向邻居 */
            public getAxisNeighbors(mapPos: IPos, Axis: cc.Vec2[]): IPos[] {
                var arr = [];
            ...
            //================================================ public
            /** 生成热力图 */
            public createHeatMap(graph: IGraph, map: IGrid[], targetPos: cc.Vec2): void {
                var open_list: INode[] = [];
            ...
            /** 留优化版本 */
            public createFullHeatMap(map: IGrid[], segments: ISegment[], targetPos: cc.Vec2): void {
            ...
            /** 生成向量图 */
            public createVectorMap(graph: IGraph, map: IGrid[], targetPos: cc.Vec2): void {
                let result: INearestPoint = {};
            ...
            /** 通过网格四方向, 生成连通图 */
            public createGraphByGrids(map: IGrid[]): {[key: string]: {[key: string]: number}} {
            ...
            /** 通过点&线段, 生成连通图 */
            public createGraphByPoints(points: IGrid[], segments: ISegment[], map: IGrid[]): IGraph {
                let graph = {};
                let rects = this.segments2rect(segments);
            ...
            //================================================ 控制点寻路特有
            /** 生成关键点 */
            public createKeypoints(map: IGrid[]): IGrid[] {
                let points = [];
                for(let i = 0; i < map.length; i++) {
                    let blocks: IGrid[] = [];
                    let selected_grid = map[i];
            ...
            /** 生成阻挡线段 */
            public createSegments(map: IGrid[]): ISegment[] {
                let segments = [];
            ...
            /** 更新图元链接状态 */
            public updateGraphPoint(graph: IGraph, points: IGrid[], rects: cc.Rect[], newPoint: IPos) {
                let point0 = newPoint;
                for(let j = 0; j < points.length; j++) {
                    let point1 = points[j];
            ...
            /** 添加图元 */
            public addGraphElement(graph: IGraph, map: IGrid[], points: IGrid[], segments: ISegment[], newPoint: IPos) {
                if (!newPoint) {
                    return;
                }
            ...
            /** 删除图元 */
            public delGraphElement(graph: IGraph, map: IGrid[], delPoint: IPos): void {
                if (!delPoint) {
                    return;
                }
